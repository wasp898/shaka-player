...
shaka.dash.CustomDashParser.prototype.parseAdaptationSet_ = function(context, elem) {
  ...
  var viewpointArray = XmlUtils.findChildren(elem, 'Viewpoint');
  var viewpointValue = undefined;
  for (var i = 0; i < viewpointArray.length; ++i) {
    var viewpointElem = viewpointArray[i];

    var scheme = viewpointElem.getAttribute('schemeIdUri');
    if (!scheme) continue;
    var value = viewpointElem.getAttribute('value');
    if (!value) continue;

    switch (scheme) {
      case 'urn:mpeg:dash:viewpoint:2011':
        var viewpointValue = parseInt(value, 10);
        break;
      default:
