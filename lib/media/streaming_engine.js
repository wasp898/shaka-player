...
shaka.media.StreamingEngine.prototype.clearBuffer_ =
  ...
  if (safeMargin) {
    ...
    var position = this.lookupSegmentPosition_(
      mediaState, playheadTime, needPeriodIndex);
    var reference = this.getSegmentReferenceIfAvailable_(
      mediaState, needPeriodIndex, position);
    shaka.log.debug('clearBuffer_: Segment duration '+(reference.endTime-reference.startTime));
    shaka.log.debug('clearBuffer_: Segment end '+reference.endTime);
    shaka.log.debug('clearBuffer_: Remove at '+(playheadTime + safeMargin));
    var duration = this.playerInterface_.mediaSourceEngine.getDuration();
