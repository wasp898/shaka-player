...
shaka.abr.CustomAbrManager.prototype.suggestStreams_ = function() {
  ...
  // TEST: Override chosenVariant to cause a switch of variants
  var CustomAbrManager = shaka.abr.CustomAbrManager;
  var sortedVariants = CustomAbrManager.filterAndSortVariants_(
    this.config_.restrictions, this.variants_);
  var playheadTime =
  ...
  //var segmentDuration = 4.003966666666667; // VR
  var segmentDuration = 1.0009916666666667;
  opt_safeMargin = segmentDuration - (playheadTime % segmentDuration);

  var switchCount = Math.floor(playheadTime / 10.0);
  var remainder = switchCount % 2;
  for (var i = 0; i < sortedVariants.length; ++i) {
    if (sortedVariants[i] == chosenVariant && i > 0) {
      chosenVariant = sortedVariants[i - remainder];
    }
  }
  opt_safeMargin = 2 * segmentDuration;
  shaka.log.debug('safe margin '+opt_safeMargin);

  var bufferedAhead =
      this.playerInterface_.mediaSourceEngine.bufferedAheadOf(
      mediaState, playheadTime);
  if (bufferedAhead < opt_safeMargin) {
    opt_clearBuffer = false;
  }
